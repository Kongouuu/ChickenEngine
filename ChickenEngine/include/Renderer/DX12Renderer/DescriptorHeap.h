...
namespace ChickenEngine
{
	...
	public:
		...
		inline static D3D12_GPU_DESCRIPTOR_HANDLE GetSrvGpuHandle(uint32_t offset) { return instance().mSrvHeap.GetGpuHandle(offset); }
		...
		inline static Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> GetSrvHeap() { return instance().mSrvHeap.Get();  }
	public:
		void Init(int swapChainBufferCount);
		void BuildDesc();
	private:
		int mSwapChainBufferCount = 0;
		const size_t srvSize = 8092;
		...
		std::vector<uint32_t> mSrvAvailable;
